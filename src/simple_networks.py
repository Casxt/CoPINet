...
class SimpleCoPINet(nn.Module):
    ...
    def forward(self, x):
        ...
        row1 = row_features[:N//2, :, :, :].view(N//2, 1, 1, self.internal_channel, H, W) # .unsqueeze(1).unsqueeze(1)#.expand(N, 1, 1, self.internal_channel, H, W)
        # row2 = row_features[N:2 * N, :, :, :].unsqueeze(1).unsqueeze(1).expand(N, 8, 1, 64, 20, 20)
        row3 = row_features[N//2:, :, :, :].view(N//2, 1, 1, self.internal_channel, H, W)# .unsqueeze(2)
