...
class SimpleCoPINet(nn.Module):
    ...
    def forward(self, x):
        ...
        input_features = self.maxpool(
            self.relu(self.inf_bn1(self.inf_conv1(prior.contiguous().view(-1, 40, 40).unsqueeze(1)))))
        input_features = input_features.view(-1, 3, 64, 10, 10)
        ...
        contrast1_bias = contrast1_bias.view(-1, 64, 1, 1).expand(-1, -1, 10, 10)
        contrast2_bias = self.contrast2_bias_trans(basis_bias)
        contrast2_bias = contrast2_bias.view(-1, 64, 1, 1).expand(-1, -1, 5, 5)

        # Perception Branch
        input_features = self.maxpool(self.relu(self.bn1(self.conv1(x.view(-1, 40, 40).unsqueeze(1)))))
        input_features = input_features.view(-1, 4, 64, 10, 10)
        ...
        row3_pre = input_features[:, 2:3, :, :, :].unsqueeze(1)  # .expand(N, 1, 1, 64, 10,10) N, 1, 64, 10, 10 -> N, 1, 2, 64, 10, 10 -> N, 1, 1, 64, 10, 10
        row3_features = torch.sum(torch.cat((row3_pre, choices_features), dim=2), dim=2).view(-1, 64, 10,
                                                                                              10)  # N, 1, 1, 64, 10, 10 -> N, 1, 64, 10, 10 -> N * 1, 64, 10, 10
        row_features = self.relu(
            self.bn_row(self.conv_row(torch.cat((row1_features, row3_features), dim=0))))

        row1 = row_features[:N, :, :, :].unsqueeze(1).unsqueeze(1).expand(N, 1, 1, 64, 10, 10)
        # row2 = row_features[N:2 * N, :, :, :].unsqueeze(1).unsqueeze(1).expand(N, 8, 1, 64, 20, 20)
        row3 = row_features[N:, :, :, :].view(-1, 1, 64, 10, 10).unsqueeze(2)
        ...
        input_features = final_row_features # + final_col_features
        input_features = input_features.view(-1, 64, 10, 10)

        res1_in = input_features.view(-1, 1, 64, 10, 10)
        ...
        res3_in = torch.nn.functional.interpolate(res2_in.view(-1, 1 * 128, 5, 5), scale_factor=8, mode='nearest',
                                                  align_corners=None)
        res3_out = self.res3(res3_in.view(-1, 1 * 128, 40, 40))
